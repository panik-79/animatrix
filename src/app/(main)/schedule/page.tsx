...
export default function SchedulePage() {
  const { activeDay, setActiveDay, items, isLoading, isError } = useSchedule();
    ...
    <div className="w-full px-3 md:px-6 pb-20 pt-2 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card/80 border border-border/80 p-4 md:p-6 rounded-3xl backdrop-blur-xl shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-xs md:text-sm text-muted-foreground pl-0.5">
            Real-time weekly episode timetables & broadcast releases.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={`No broadcast episodes scheduled for ${
            DAYS_MAP.find((d) => d.id === activeDay)?.label
          }.`}
          action={
                ...
                <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-950">
                  ...
                  {anime.isInUserLibrary ? (
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-1.5 rounded-full bg-black/60 hover:bg-primary text-white transition-all cursor-pointer opacity-0 group-hover:opacity-100 shadow-md"
